 ...
 *
 * QUARANTINED fora de `src/` desde 12/ago/2026 (achado P-16, auditoria de
 * segurança de 11/ago/2026). Este componente roda `new Function(...)` sobre
 * código não confiável e carrega runtimes (Pyodide, esbuild-wasm) de CDNs
 * que NÃO estão na CSP declarada (`script-src`/`connect-src`, ver
 * frontend/next.config.ts) — se algum módulo voltasse a importá-lo, seria
 * execução de código arbitrário no navegador fora da política de segurança
 * do site. Confirmado sem nenhum importador ativo em `src/` no momento da
 * quarentena. Fica aqui como rascunho/referência — `tsc`/`eslint`/`next
 * build` não o alcançam (fora de `src/`, e `drafts/` está em `exclude` no
 * tsconfig). Antes de religar: sandboxing real (iframe com CSP própria,
 * isolado do resto da página) é pré-requisito, não opcional — ver
 * PENDENCIAS.md.
 *
