...
func NewLeaderboardRepo(pool *pgxpool.Pool) *LeaderboardRepo { return &LeaderboardRepo{pool: pool} }

func (r *LeaderboardRepo) UpsertXP(ctx context.Context, userID shared.UserID, weekStart time.Time, xp int) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO leaderboard (week_start, user_id, xp_gained) VALUES ($1,$2,$3)
		 ON CONFLICT (week_start, user_id) DO UPDATE SET xp_gained = leaderboard.xp_gained + $3`,
		weekStart, userID.String(), xp,
	)
	return err
}
...
func (r *LeaderboardRepo) GetWeekly(ctx context.Context, weekStart time.Time, limit int) ([]domlb.RankEntry, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT lw.user_id, u.name, lw.xp_gained,
		       RANK() OVER (ORDER BY lw.xp_gained DESC) AS rank
		FROM leaderboard lw
...
// GetUserRank retorna o rank do usuário na semana.
// Subquery ranqueia toda a semana; outer WHERE filtra o usuário sem perder a posição.
// Antes: filtro era dentro da subquery, quebrava o RANK() (sempre 1).
func (r *LeaderboardRepo) GetUserRank(ctx context.Context, userID shared.UserID, weekStart time.Time) (int, error) {
	var rank int
	err := r.pool.QueryRow(ctx, `
		SELECT rank FROM (
			SELECT user_id, RANK() OVER (ORDER BY xp_gained DESC) AS rank
			FROM leaderboard
			WHERE week_start = $1
		) ranked
		WHERE user_id = $2
	`, weekStart, userID.String()).Scan(&rank)
	if errors.Is(err, pgx.ErrNoRows) {
		return 0, nil
	}
	return rank, err
...
func (r *LeaderboardRepo) SetOptIn(ctx context.Context, userID shared.UserID, optIn bool) error {
	if optIn {
		_, err := r.pool.Exec(ctx,
			`INSERT INTO leaderboard_opt_ins (user_id) VALUES ($1) ON CONFLICT DO NOTHING`,
			userID.String(),
		)
		return err
	}
	_, err := r.pool.Exec(ctx, `DELETE FROM leaderboard_opt_ins WHERE user_id = $1`, userID.String())
	return err
}

func (r *LeaderboardRepo) IsOptedIn(ctx context.Context, userID shared.UserID) (bool, error) {
	var exists bool
	err := r.pool.QueryRow(ctx,
		`SELECT EXISTS(SELECT 1 FROM leaderboard_opt_ins WHERE user_id = $1)`, userID.String(),
	).Scan(&exists)
	return exists, err
}
